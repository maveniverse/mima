 ...
 * </ul>
 * <p>
 * Differences between {@link #createResolutionClient(RepositorySystemSession, RemoteRepository)} and {@link #createDeploymentClient(RepositorySystemSession, RemoteRepository)}
...
public class MavenHttpClient4FactoryImpl {
    ...
    public HttpClientBuilder createResolutionClient(RepositorySystemSession session, RemoteRepository repository) {
        ...
        List<RemoteRepository> repositories =
                repositorySystem.newResolutionRepositories(session, Collections.singletonList(repository));
        if (repositories.size() != 1) {
            ...
            return createClient(session, remoteRepository);
        }
    }
    ...
    public HttpClientBuilder createDeploymentClient(RepositorySystemSession session, RemoteRepository repository) {
        requireNonNull(session, "session");
        requireNonNull(repository, "repository");
        return createClient(session, repositorySystem.newDeploymentRepository(session, repository));
    }
    ...
    protected HttpClientBuilder createClient(RepositorySystemSession session, RemoteRepository repository) {
        ...
        try (AuthenticationContext repoAuthContext = AuthenticationContext.forRepository(session, repository);
            ...
            ServiceUnavailableRetryStrategy serviceUnavailableRetryStrategy =
                    new ResolverServiceUnavailableRetryStrategy(
    ...
    protected static class ConnMgrConfig {
        ...
        protected ConnMgrConfig(
            ...
            this.cipherSuites = split(get(session, CIPHER_SUITES));
            this.protocols = split(get(session, PROTOCOLS));
